...
corresponding Codex-side artifact. Naming conventions:
    ...
    plugins/<plugin>/agents/<name>.md       -> plugins/<plugin>/.codex/agents/<name>.toml
    plugins/<plugin>/skills/<name>/         -> plugins/<plugin>/.agents/skills/<name>/
    plugins/<plugin>/commands/<name>.md     -> plugins/<plugin>/.agents/skills/cmd-<name>/
...
def test_every_claude_agent_has_codex_counterpart(plugin: Plugin) -> None:
    ...
    assert not missing, (
        ...
        f"plugins/{plugin.value}/.codex/agents/: {sorted(missing)}"
    )
...
def test_every_claude_skill_and_command_has_codex_counterpart(plugin: Plugin) -> None:
    ...
    assert not missing, (
        ...
        f"plugins/{plugin.value}/.agents/skills/: {sorted(missing)}"
    )
