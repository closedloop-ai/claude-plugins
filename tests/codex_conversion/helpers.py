...
def codex_skill_universe() -> frozenset[str]:
    ...
    for plugin in Plugin:
        skills_dir = REPO_ROOT / "plugins" / plugin.value / ".agents" / "skills"
...
def _codex_agent_path(plugin: Plugin, name: str) -> Path:
    return REPO_ROOT / "plugins" / plugin.value / ".codex" / "agents" / f"{name}.toml"
...
def _codex_skill_path(plugin: Plugin, name: str) -> Path:
    return REPO_ROOT / "plugins" / plugin.value / ".agents" / "skills" / name / "SKILL.md"
