...
#### Changed
- `CODE_INTEL_ALLOWED` is now set to `false` when `scope.json` → `worktree_path` is non-empty, instead of when `review_root` is non-empty. `review_root` is now populated on every run, so the previous key would have disabled code intelligence for every review; `worktree_path` is set only when a PR-head worktree separates the reviewed source from the operator's checkout. Updated in the `spawn-reviewers` skill and the `stage_03_resolve_scope` note in `start.md`.
