...
- **Verification subagent (old `/fix` Step 2 — parallel verifier subagents per finding) REMOVED entirely.** Verification is the code-review pipeline's job (`cmd_verify_prepare` + `cmd_verify_consolidate`, PLN-722); `/fix` no longer re-implements it. The skill now trusts the envelope's verdict state: `verified[]` is treated as audited (no second-opinion pass), `pending_verification[]` entries route to **manual-surface** with the new `pending_verification.md` template that explains the operator's options (re-run the review, `--re-assert`, or fix by hand). Eliminates ~50 lines of parallel verifier prompt that would have drifted out of sync with the canonical verifier prompt over time, and removes the v2.12.x design's silent LLM cost on every `/fix` run.
