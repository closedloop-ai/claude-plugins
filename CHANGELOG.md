...
#### Fixed
- Fixed judges agents path resolution in `run-loop.sh` to support monorepo, cache, and marketplace installation layouts via a three-level fallback strategy (`CLOSEDLOOP_JUDGES_AGENTS_DIR` env override → non-versioned sibling → latest semver-versioned sibling)
