...
#### Changed
- `cmd_extract_signals_consolidate` reads `manifest.status` and no-ops when it is `cache_hit` (prepare already wrote the canonical `extract_signals.json` directly; re-reading from cache would duplicate prepare's work and require an agent output file that doesn't exist on cache hit). On a no-op the cmd writes a one-line `{"status": "cache_hit", "output_path", "cache_key"}` JSON to stdout and returns 0.
