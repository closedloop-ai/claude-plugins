...
- Bash test suite under `plugins/code/hooks/tests/` (5 files, 109 tests total) covering `tool` event emission, `skill` event emission with `tool_input.skill` → `tool_input.command` fallback (plus explicit `skill > command` priority and `neither field present` cases), `spawn` event emission for `Agent` calls, fail-open contract that exercises the *real* hooks with malformed JSON (rather than self-validating wrappers), and `test_correlation.sh` covering real pre→post correlation, sentinel-attribution-wins-over-env-drift, corrupt-sentinel suppression, missing-correlation-id silent skip, pre-hook iteration normalization, and post-hook env-fallback iteration normalization.

#### Notes
- These hooks rely on Claude Code's `PreToolUse`/`PostToolUse` lifecycle and on `tool_use_id` correlation in the hook payload. There is no documented `plugin.json` field for pinning a minimum Claude Code version (the published manifest schema only supports `name`, `version`, `description`, `author`, `homepage`, `repository`, `license`, `keywords`, `dependencies`, and component-path fields). On a Claude Code build that lacks the expected payload, the hooks degrade gracefully — both `pre-tool-use-hook.sh` and `post-tool-use-hook.sh` exit 0 silently when both `tool_use_id` and `tool_call_id` are absent, so no orphan sentinels accumulate and the Loop is unaffected.
- `post-tool-use-hook.sh` `OK` derivation type-guards `tool_response` against non-object scalars: `if (.tool_response | type) != "object" then "true" elif ... end`. Without this guard, a tool returning a string or array as `tool_response` would have caused the entire `eval "$(jq -r ...)"` block to fail, leaving `TOOL_NAME`, `SESSION_ID`, etc. unset and silently skipping emission with an orphaned sentinel.
