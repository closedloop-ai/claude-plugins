...
#### Added
- `ignore_case` field on `path_pattern` triggers in `coverage[]` rules. Default `False` (canonical rules keep explicit case-sensitive semantics — operators who write `src/Foo` mean `Foo`). The legacy migration sets `ignore_case: True` so migrated rules preserve the legacy `substring.lower() in path.lower()` semantics.
- `_build_run_plan_stages` `stage_11_extract_signals` now invokes the actually-shipped `extract-signals-prepare` subcommand (Phase 1 ships a two-step prep/consolidate flow, not a single command). Stage's `expected_outputs` references both `extract_signals_manifest.json` and `extract_signals.json` rather than the never-produced `signals.json`.
- `prior_migrated_pruned` field in `migrate-critic-gates` summary output. Counts the `_migrated_from="moduleCritics"` entries pruned from `coverage[]` before appending the freshly migrated set, so operators can see what idempotency claimed.
- 16 PR #124 regression tests across 7 new classes pinning every fix below.

#### Fixed
- **Case-sensitive migration patterns broke legacy semantics (PR #124, Bug Hunter B HIGH + thadeusb).** Legacy `route` matched `pattern.lower() in path.lower()`. The migration wrapped substrings as `**<sub>**` and ran them through `_glob_to_regex`, which compiles case-sensitive regex (`^.*sub.*$`, no `IGNORECASE`). A `moduleCritics` pattern `graphql` that used to match `src/GraphQL/schema.ts` would have silently dropped the reviewer from coverage. The `_trigger_fires` `path_pattern` branch now honors `ignore_case`; the migration emits `ignore_case: True` on every migrated trigger.
- **`stage_14_resolve_coverage` passed `--signals` but Phase 2 CLI registers `--extract-signals` (PR #124, devops-architect HIGH).** Pre-declared stages in the prepare-run manifest used placeholder flag names from before Phase 1/2 shipped. Once Phase 4 flipped the `enabled: False` gate the orchestrator would have crashed with `unrecognized arguments`. Stages 11, 14, and 15 now reference real CLI surfaces; `stage_11_extract_signals` writes to `extract_signals.json` (not `signals.json`).
- **`_validate_coverage_rule` silently accepted unknown `change_class` values (PR #124, Unified Auditor MED).** The validator checked trigger TYPE membership but not the `class` field on `change_class` triggers. A typo like `scheme_change` would have passed validation and silently never fired — invisible misroute. Validator now checks `class` against `COVERAGE_CHANGE_CLASSES` and emits a warning enumerating valid values.
- **Determinism-downgrade warning fired even when the rule never matched (PR #124, Bug Hunter A MED).** Operators saw `"Rule for reviewer 'X' was required=true ... downgraded to best-effort"` warnings for rules whose triggers didn't fire — claiming a reviewer was "downgraded" when it actually wouldn't have been selected at all. The downgrade decision still happens before the match check (so the required flag is correct when the rule selects), but the warning is now emitted only after the rule actually matches.
- **`_validate_coverage_rule` docstring claimed to emit the required-with-only-LLM-signal warning, but the check was absent from the function body (PR #124, Bug Hunter B + thadeusb MED).** The runtime invariant lives in `resolve_coverage` (where the match-vs-no-match distinction lives); the validator catches edit-time structural problems only. Docstring corrected.
- **`TestTriggerFires._empty()` helper was dead code — never called by any of the 16 test methods (PR #124, Bug Hunter B MED).** Removed.
- **`cmd_resolve_coverage` output write lacked try/except, violating its own docstring contract (PR #124, Unified Auditor MED).** The docstring promised exit 0 on structurally valid runs and exit 1 on file-read failure, but an unwritable `output_path` would have propagated `OSError`. Now matches the documented contract — returns 1 with a clear stderr message on write failure.
- **`--in-place` and `--output` not enforced as mutually exclusive by argparse (PR #124, devops-architect MED).** The `cmd_migrate_critic_gates` body resolved precedence internally (`output` won), but a user passing both got no warning. Now uses `add_mutually_exclusive_group()` — argparse rejects the combination with exit code 2.
- **Migration was not idempotent — running `--in-place` twice would duplicate every rule (PR #124, thadeusb).** `cmd_migrate_critic_gates` appended migrated entries onto existing `coverage[]` without checking for prior `_migrated_from="moduleCritics"` rows. Two runs → coverage[] doubled. (Resolved plan stayed correct via `resolve_coverage` dedup-by-reviewer-name, but the file grew on each run.) The migrator now prunes any prior `_migrated_from="moduleCritics"` entries from `coverage[]` before appending the freshly migrated set; operator-edited canonical entries (no marker) are preserved.
