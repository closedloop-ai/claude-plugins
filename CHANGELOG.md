...
#### Changed
- `CODE_INTEL_ALLOWED` is now `true` only when `scope.json` → `review_root` resolves (realpath) to the git toplevel of the session's primary working directory — the directory the Claude Code session was started in, which is what session MCP servers index — and `false` otherwise, including when either side cannot be determined. It was previously `false` whenever `review_root` was non-empty, which would disable code intelligence on every run now that `review_root` is always populated. The `spawn-reviewers` gate directs the orchestrator to use the session's primary working directory, not the directory a helper process ran in. Code intelligence is now off for a PR-head worktree under `<CR_DIR>` and for a branch review run from a separate git worktree, and stays on for branch, staged, and file-path review from the primary checkout, local PR review already at the head, and GitHub mode. Updated in the `spawn-reviewers` skill and the `stage_03_resolve_scope` note in `start.md`.
