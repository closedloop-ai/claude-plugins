...
#### Changed
- `CLOSEDLOOP_PERF_V2=1` env-var gate removed from the rest of the PRD-254 producer/orchestrator surface area so every perf.jsonl event row carries `command:` in production. `run-loop.sh::emit_perf_event()` now adds `command:` (defaulting to `"interactive"` when `CLOSEDLOOP_COMMAND` is unset) unconditionally, so phase, iteration, and pipeline_step events all carry the field; `record_run.sh` no longer short-circuits at the top of the script and emits its singular `run` event on every fresh-start Loop; `record_phase.sh` drops the `--arg perf_v2` ternary and always includes `command:` in the emitted JSON. Safety properties are unchanged — the additive event schema means existing consumers ignore the field, and the fail-open `trap 'exit 0' ERR` contract still applies. The earlier gate was permanently off in production because closedloop-electron ships claude-plugins bundled in the desktop app and end users have no way to set runtime env vars. `test_record_run.py` and `test_record_phase.py` replace their gate-on/gate-off classes with their inversions (`TestRecordRunUnconditional` and `TestRecordPhaseCommandFieldAlwaysPresent`) that pin the no-gate contract; `test_subagent_stop_hook.py` drops stale `CLOSEDLOOP_PERF_V2: "1"` env_extra entries that were no-ops after the earlier hook gate removal. `plugins/code/README.md`'s `record_run.sh` description now reads "Emitted unconditionally and fails open" in place of the prior "Gated behind `CLOSEDLOOP_PERF_V2=1`" wording.
