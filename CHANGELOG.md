...
#### Notes
- The `minClaudeCodeVersion: "1.0.33"` field in `plugin.json` is **advisory only** — there is no precedent for this field name in any plugin manifest in this repo, and Claude Code's plugin loader does not appear to enforce it. Sister PR #69 introduced `claudeCodeMinimumVersion` for the same intent. Neither name is verified canonical against an external schema; whichever matches Claude Code's spec should be applied to both PRs in a follow-up. The field documents the required hook payload contract regardless of whether it's enforced.
