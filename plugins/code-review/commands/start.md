...
- **Batch deterministic helper stages.** Consecutive `helper`-kind stages with no `GATES` entry firing between them, no branching-gate boundary (A/B/C/D), no `agent_fleet`/`present`/singleton-dispatch stage between them, and no arg that depends on a value an in-batch predecessor printed to stdout MAY be chained in a single `Bash` call (`cmd1 && cmd2 && …`), each redirecting stdout per its `stdout` field. After the batched call, confirm every chained stage's `expected_outputs` and run any pending gates in order; if a chained command exits non-zero, stop at it and apply that stage's `on_failure`.
