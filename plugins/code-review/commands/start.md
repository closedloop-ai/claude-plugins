...
- **stage_14_resolve_coverage** (PLN-725 Phase 2, wired in Phase 4): runs `resolve-coverage` against `coverage` rules + `extract_signals.json` + diff data. Writes the deterministic pre-critic plan into `<CR_DIR>/coverage.json` (`initial` section — Phase C v2.27.0; pre-v2.27.0 produced a standalone `coverage_plan_initial.json`). `depends_on: ["stage_11b_extract_signals_consolidate"]` so the signals input is guaranteed to exist. `on_failure: continue_with_coverage_gap` — Phase 4 wires this for telemetry; downstream stages read `coverage.json.initial` directly.
...
| `stage_15_coverage_critic` | `<CR_DIR>/coverage.json` (`critic` section) — Phase C v2.27.0; pre-v2.27.0 used standalone `coverage_critic_manifest.json`. |
