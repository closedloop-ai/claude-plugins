...
7. **Run gates.** After completing a stage, scan `GATES` for any entry whose `after_stage` matches the stage just finished. Each gate checks `outputs` exist and are well-formed. A gate may also declare `required_sections`: a map from a file path to a list of top-level dict keys that MUST be present in that file. This distinguishes "the file exists on disk" from "the stage that owns this section actually populated it." `coverage.json` exists from `stage_14` onward (sections accumulate), so a bare file-existence check after `stage_16` would fire-true even if stages 15/15b/15c/16 all failed to populate their sections. When `required_sections` is declared, the walker must load each listed file as JSON and assert every listed section key is present as a top-level dict key before treating the gate as passed. `on_failure_action` follows the same `abort | continue | emit_coverage_gap` semantics for both checks.
