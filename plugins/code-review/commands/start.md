...
- **stage_19b_derive_spawn_spec** (PLN-725 Phase 8): runs `derive-spawn-spec`. Reads `<CR_DIR>/coverage_plan.json` (post-arbitrate), `<CR_DIR>/partitions.json`, and `<CR_DIR>/route.json` and writes `<CR_DIR>/spawn_spec.json` — a flat list of agent descriptors keyed by `agent_id` (e.g. `bha_p0`, `bhb`, `auditor`, `premise`, `domain_0`, `fast`) carrying `reviewer`, `model`, `partitioned`, `patches_file`, `source`, `bucket`, and (for BHA) `partition_id` + `is_test_only`. The fast-path branch from Gate B is honored (`fast_path: true` → single `fast` agent, bucket walk skipped). The Phase 7 BLOCKING gate's `gated_by_verify` flag is propagated to the spec so presenters can surface that arbitration was bypassed; review still runs against the (unbudgeted) input plan. `on_failure: continue` — a derive failure writes a sentinel spec with `arbitrate_status: "fallback"`, which the stage_20 orchestrator interprets as "ignore the spec, walk the static Reviewer Fleet table below." A missing or unreadable `coverage_plan.json` produces the same fallback sentinel, never an abort.
- **stage_20_spawn_reviewers**: agent_fleet stage. Dispatch to the "Reviewer Fleet" section below. PLN-725 Phase 8: the orchestrator reads `<CR_DIR>/spawn_spec.json` first and dispatches one Task per agent descriptor (using the `agent_id`, `reviewer`, `model`, and `patches_file` from the spec). If `spawn_spec.json` is missing or marks `arbitrate_status: "fallback"`, walk the static reviewer table in the "Reviewer Fleet" section instead — a derive failure must never block review.
...
**PLN-725 Phase 8 — spawn-spec consumption (preferred path).** Before walking the static tables below, Read `<CR_DIR>/spawn_spec.json`. If the file exists and `arbitrate_status != "fallback"`, dispatch one Task per entry in `agents[]`, using the descriptor fields directly:

- `agent_id` → orchestrator-assigned ID; agent writes to `<CR_DIR>/agent_{agent_id}.json`.
- `model` → resolved per-agent model string (already accounts for BHA test-only routing and route.json overrides — do not re-derive).
...
- `source` ∈ {`core`, `critic`, `fast_path`} → picks the per-agent prompt suffix below (`bug_hunter_a` → BHA, `bug_hunter_b` → BHB, `unified_auditor` → Auditor, `premise_reviewer` → Premise, `critic` → Domain Critic, `fast_path` → Fast Path).
...
The static tables, model selection notes, and partition-to-agent mapping below remain authoritative for the **fallback path** (when `spawn_spec.json` is absent or marks `arbitrate_status: "fallback"`) and for human inspection of the canonical fleet shape.
