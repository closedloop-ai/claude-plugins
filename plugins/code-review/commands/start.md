...
- **stage_15_coverage_critic** (PLN-725 Phase 3, wired in Phase 4): runs `coverage-critic-prepare`. Writes `<CR_DIR>/coverage_critic_manifest.json`. On `status: "cache_hit"` or `status: "skipped"` (the `--no-critic` flag, not wired in Phase 4), prepare wrote `<CR_DIR>/coverage_plan.json` itself — no agent runs, downstream `stage_15b` no-ops. On `status: "needs_agent"`, the walker invokes the "PLN-725 Single-Agent Dispatch" protocol with the manifest's paths and target `pln725_coverage_critic.json`. `on_failure: continue` — coverage-critic failure surfaces as `critic_status: "fail_closed"` on the final plan; the deterministic floor from stage_14 still routes reviewers correctly.
...
- **stage_15c_verify_coverage** (PLN-725 Phase 6): runs `verify-coverage`. Deterministic post-LLM verifier — reads `<CR_DIR>/coverage_plan.json` (final, post-consolidate), `<CR_DIR>/coverage_plan_initial.json` (pre-critic), and `<CR_DIR>/available_reviewers.json` (roster), then checks the closed-vocabulary, additive-only, best-effort-only-critic, evidence-required, 5-cap, and no-duplicates contracts. Writes `<CR_DIR>/coverage_verify.json` with `verdict: "PASS"` or `verdict: "BLOCKING"` and a `violations[]` list keyed by check name. On BLOCKING also emits a HIGH system-marker finding to `<CR_DIR>/agent_coverage-verify-blocking.json` so the run summary surfaces the failure. Phase 6 rollout: observational only — exit 0 on both verdicts and `on_failure: continue` so the walker doesn't halt. Phase 7 will gate stage_16 / stage_20 on a PASS verdict by reading `coverage_verify.json` from those downstream stages (not by changing the verifier's exit code). The closed-vocabulary check is bypassed when the roster file is missing or empty (no-roster skip path); missing inputs degrade to PASS with an advisory `input` violation so verification stays observational instead of vacuously blocking.
