...
1. Start from `CODE_INTEL_ALLOWED = false`. Step 2 is the only thing that sets it `true`.
2. **Set `CODE_INTEL_ALLOWED = true` only when the reviewed root IS the session's primary checkout.** Resolve both sides to real paths (`realpath`) and compare: (a) `<CR_DIR>/scope.json` → `review_root`, and (b) the git toplevel of the SESSION's primary working directory — the directory this Claude Code session was started in, as your session environment states it — computed with `git -C <session primary working directory> rev-parse --show-toplevel`. Use the session's primary working directory, NOT whatever directory a helper process happened to run in: the cwd recorded in `setup.json`, the directory `resolve-scope` ran from, and a Bash call's cwd can all differ from it. `true` only on an exact match; `false` on a mismatch AND whenever either side cannot be determined (empty or missing `review_root`, the primary working directory is not inside a git worktree, or the git call fails). The reason: a code-intelligence MCP server connected to the session indexes the session's primary checkout. When `review_root` is any other tree, that index describes a different branch or commit than the source the agents Read/Grep, and reviewers (Bug Hunter B, Impact Analyzer, Design Critic, fast-path) would surface that other tree's symbols into findings. That covers a branch review run from a separate git worktree (for example a per-chunk campaign worktree) and subsumes PR-head worktree isolation — that worktree lives under `<CR_DIR>`, so it never matches. Re-indexing per review is out of scope, so grep-only is the safe behavior. The common cases keep code intelligence on, because there `review_root` is the primary checkout's own toplevel: branch, staged, and file-path review run from the primary checkout; local PR review already at the PR head; and GitHub mode, where the runner checks out the head in its working directory. This holds for every substrate, present or future — it is a property of the review, not of the server.
