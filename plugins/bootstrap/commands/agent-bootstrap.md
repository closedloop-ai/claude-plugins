...
- `--target-command <name>` - Target command to generate agents for (default: "impl-plan")
...
- `--output-dir <path>` - Where to write generated agents (default: `.claude/agents/prd2plan/`)
...
- `--strategy backup|skip|overwrite|interactive` - How to handle existing agent files
  - `backup` (default): Backup existing files to `.claude/agents/prd2plan/backup-{timestamp}/` before overwriting
...
Analyzes candidate agents and decides which should be split into specialists based on complexity and breadth. Generates critic-gates.json configuration for the impl-plan workflow.
...
**Produces:** `.claude/agents/prd2plan/*.md`, `.claude/agents/prd2plan/.bootstrap-metadata.json` (durable); working artifacts under `$RUN/...`.

### Phase 7: Agent Prompt Validation

Validates generated prompts for structure, YAML headers, valid colors, and artifact contracts. By default, validation is scoped to the agents listed in `.claude/agents/prd2plan/.bootstrap-metadata.json`. Use `--legacy` to sweep all agents.

**Produces:** `$RUN/synthesis/agent-validation.json`

### Phase 8: Final Validation

Validates complete agent suite and DAG integrity.

**Produces:** `validation-report.json`, `bootstrap-report.md`

## Output

All working files written to `.claude/bootstrap/<timestamp>/` (referenced as `$RUN`):
...
Generated agents written to `.claude/agents/prd2plan/`:
...
- `.claude/settings/critic-gates.json` - Critic selection rules for impl-plan workflow (safe write)

Note: `.claude/commands/prd2plan/impl-plan.json` is a static workflow definition file that should be present in the repository/distribution, not generated by bootstrap.
...
4. Test with: `/impl-plan --prd <your-prd.md>`
