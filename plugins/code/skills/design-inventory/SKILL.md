...
3. Ticket bodies MUST be self-contained: an implementing agent in a fresh worktree, with ONLY the ticket, must be able to mirror the design. `build-design-pack` embeds the unit's design source and sliced CSS directly in `ticket-body-ui.md` / `ticket-body-api.md` so the body needs nothing external. NEVER write a ticket body that depends on the run workdir, the export zip, or a "design pack" path/attachment the implementer will not have - those are reviewer-local and are never delivered. If you hand-author or edit a ticket body, embed the source the same way.
