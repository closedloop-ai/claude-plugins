...
5. For modified/new units, itemize every difference as findings, grouped by category. Every finding's `spec.summary` MUST state the concrete visual geometry of the change - key dimensions, spacing/padding/gap values, alignment, color tokens or the raw values when no token exists, and typography (size/weight/line-height) - NOT intent-level prose. ("Topbar is 56px tall, sticky, 16px horizontal padding, title at 18px/600, uses `--primary`" - not "modernize the header.") `state.summary` likewise states the concrete current geometry. These summaries become the ticket's acceptance criteria VERBATIM in generated tickets (Stage C `build-design-pack`), so an implementer with no access to the design source must be able to reproduce the change from your summary plus its refs alone. Group by category:
...
9. **Selectors and refs for visual anchoring.** In each finding's `spec` block, include `selectors`: 1-3 CSS class selectors taken verbatim from the design source that visually anchor the change (e.g. `.sess-topbar`, `.sess-awaiting-chip`). The orchestrator uses them to capture highlighted screenshots of the live design, so prefer the most specific stable class on the changed element; omit the field when no stable class exists (e.g. pure behavior with no dedicated element). Make `spec.refs` and `state.refs` LINE-PRECISE - each ref is `path:line` pointing at the exact line in the design source (spec.refs) or current web-ui (state.refs) where the geometry lives, never a bare filename. Stage C concatenates state.refs + spec.refs into a "Refs" sub-bullet under each acceptance criterion in the generated ticket, so a vague or line-less ref leaves the implementer unable to locate the change in the embedded source.
