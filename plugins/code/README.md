...
Staged pipeline for inventorying a Claude Design export into reviewable findings, a human decision gate, and DRAFT ticket generation. Stage A extracts the zip, runs parallel `design-unit-analyst` agents per unit (screens, regions, standalone components), emits schema-validated findings, a report, and an HTML review page. Stage B is human review producing `decisions.json`. Stage C generates DRAFT feature tickets with design packs only for accepted units. Invoked via the `code:design-inventory` skill when users request a design handoff, design inventory, or ticket generation from a design review. Scripts are TypeScript under `skills/design-inventory/scripts/src/` with committed `dist/` bundles.
